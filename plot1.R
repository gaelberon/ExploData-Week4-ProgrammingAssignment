...
## The R file 'plot1.R' aims to answer the following question: 
...
## Compute dataset
sumPerYear <- with(NEI, tapply(Emissions, year, sum, na.rm = TRUE))

## Initialize a plotting area with 1 row and 1 column
par(mfrow = c(1,1))

## Plot dataset
barplot(height = sumPerYear/10^6,
        names.arg = names(sumPerYear),
        main = "PM2.5 Emissions for all US",
        xlab = "Year",
        ylab = "Amount of PM2.5 emitted (10^6 tons)")

################################################################################
## 3. Store the plot into the file plot1.png
dev.copy(png,
         file = 'plot1.png',
         width = 480,
         height = 480,
